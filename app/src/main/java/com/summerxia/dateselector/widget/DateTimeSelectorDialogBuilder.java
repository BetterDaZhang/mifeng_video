...
public class DateTimeSelectorDialogBuilder extends NiftyDialogBuilder implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getWindow().setBackgroundDrawableResource(R.drawable.date_edit_dialog_coner);
	}
	...
	private void initDialog() {
		rlCustomLayout = (RelativeLayout) LayoutInflater.from(context).inflate(
				R.layout.date_dialog_layout, null);
	...
	// 设置dialog属性，修改title的属性也可以在top_left_right_corner.xml中
	private void setDialogProperties() {
		int width = DateUtils.getScreenWidth(context);
		this.withDialogWindows(width, LayoutParams.WRAP_CONTENT)
				.withTitleColor("#FFFFFF").setDialogClick(this)
				.withPreviousText("取消").withPreviousTextColor("#3598da")
				.withDuration(100).setPreviousLayoutClick(this)
				.withNextText("完成").withMessageMiss(View.GONE)
				.withNextTextColor("#3598da").setNextLayoutClick(this)
