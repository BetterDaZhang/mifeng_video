...
public enum SizeConverter {
	 ...
	 * 
	 * @param size
	 *            要转换的大小, 注意是浮点数, 不要以整形的方式传入, 容易造成溢出. (如:
	 *            1024*1024*1024*1024*1024会溢出, 使结果为0, 因为它先将结果以int相乘后再转换为float;
	 ...
	 *            从哪个单位开始
	 * @param size
	 *            存储大小, 注意是float, 不要以整形的形式传入, 否则会溢出(如:1024*1024这种,
	...
	/***
	 * <p>
	 * 存储大小单位间的转换, 如果转换后小数部分为0, 则去除小数部分. 注意该方法的最大单位为PB, 最小单位为B,
	 * 任何超出该范围的单位最终会显示为**.
	 * </p>
	 * 
	 * @param unit
