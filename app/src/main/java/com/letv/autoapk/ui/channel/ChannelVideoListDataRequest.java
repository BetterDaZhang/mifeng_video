...
class ChannelVideoListDataRequest extends StringDataRequest {

	public ChannelVideoListDataRequest(Context context) {
		super(context);
	}

	@Override
	protected String getUrl() {
		return "/channel/androidChannelDetail";
	}
		...
		if (statusCode == 0) {
			...
			for (int i = 0; i < array.length(); i++) {
				...
				for (int j = 0; j < videoArray.length(); j++) {
					...
					vedioInfo
							.setSuperScripName(StringEscapeUtils.unescapeHtml4(videoJson.optString("superscriptName")));
