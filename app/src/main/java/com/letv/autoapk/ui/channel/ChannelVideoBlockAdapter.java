...
class ChannelVideoBlockAdapter extends BaseAdapter {
	...
	public ChannelVideoBlockAdapter(BaseActivity ctx, Fragment fragment, ArrayList<DisplayBlockInfo> infos,
			Handler handler) {
		this.mactivity = ctx;
		...
		public void loadingData(DisplayBlockInfo info) {
			...
			if (null != info && null != info.getVideoList() && !info.getVideoList().isEmpty()) {
				for (int i = 0; i < info.getVideoList().size(); i++) {
					...
					hotTitle.setTextColor(mactivity.getResources().getColor(R.color.code3));
					//
				...
				public void onClick(View v) {
					if (blockInfo.getBlockDetailId() != null && !blockInfo.getBlockDetailId().isEmpty()
						...
						Fragment describeFragment = Fragment.instantiate(mactivity,
								ChannelVideoBlockFragment.class.getName());
