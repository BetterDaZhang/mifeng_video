...
class RecommendVideoBlockAdapter extends BaseAdapter {
	private ArrayList<DisplayBlockInfo> recommendBlockInfos;
	...
	public RecommendVideoBlockAdapter(BaseActivity ctx, Fragment fragment, ArrayList<DisplayBlockInfo> infos,
			Handler handler) {
		this.mactivity = ctx;
	...
	protected View getMyView(int position, View convertView, ViewGroup viewGroup) {
		if (convertView == null) {
			if (getItemViewType(position) == TYPE_SUBJECT_TYPE) {
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_headline_vplayout_block,
						viewGroup, false);
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_block_head_item, viewGroup,
						false);
				BlockHeadHolder blockHeadHolder = new BlockHeadHolder(convertView);
				convertView.setTag(blockHeadHolder);
			} else if (getItemViewType(position) == TYPE_TWO_LANDSCAPE_TYPE) {
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_two_lands_item, viewGroup,
						false);
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_one_lands_item, viewGroup,
						false);
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_three_ver_item, viewGroup,
						false);
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_block_head_item_more, viewGroup,
						false);
				...
				convertView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_default_item, viewGroup, false);
		...
		if (getItemViewType(position) == TYPE_SUBJECT_TYPE) {
			...
			if (tag instanceof BlockHeadHolder) {
				...
				Toast.makeText(mactivity, "recommend head convertiew failed", Toast.LENGTH_LONG).show();
			}
		} else if (getItemViewType(position) == TYPE_TWO_LANDSCAPE_TYPE) {
			...
			if (tag instanceof OneLandsViewHolder) {
				...
				Toast.makeText(mactivity, "one convertiew failed", Toast.LENGTH_LONG).show();
			}
		} else if (getItemViewType(position) == TYPE_THREE_VERTICAL_TYPE) {
			...
			moreTitle.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (blockInfo.getBlockDetailId() != null && !blockInfo.getBlockDetailId().isEmpty()
						...
						Fragment describeFragment = Fragment.instantiate(mactivity,
								RecommendVideoBlockFragment.class.getName());
						Bundle bundle = new Bundle();
						bundle.putString("pageId", blockInfo.getBlockDetailId());
						bundle.putString("pageName", blockInfo.getBlockMoreName());
						describeFragment.setArguments(bundle);
						transaction.hide(fragment);
						transaction.add(R.id.container, describeFragment, "recommendVideoBlockFragment");
	...
	class CirculateHolder {
		...
		private void loadingData(List<DisplayVideoInfo> scrollList) {
			...
			for (int i = 0; i < scrollList.size() + 2; i++) {
				View rowView = LayoutInflater.from(mactivity).inflate(R.layout.recommend_headline_vpitem, null);
