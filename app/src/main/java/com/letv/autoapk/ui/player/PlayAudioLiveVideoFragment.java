...
public class PlayAudioLiveVideoFragment extends BaseFragment {
	private static final String TAG = PlayAudioLiveVideoFragment.class.getSimpleName();
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			...
			if (menuid == R.id.control_ratetype) {
				...
				if (ratetypeid != null) {
					...
					MyApplication.getInstance().putString(PlayConst.DEFINATIONTYPE, String.valueOf(ratetypeid));
				}
			}
	...
	protected View setupDataView() {
		...
		View view = LayoutInflater.from(mActivity).inflate(R.layout.play_audio_livemain, null);
		audioImg = (NetImageView) view.findViewById(R.id.play_audio_img);
		audioImg.setDefaultImageResId(R.drawable.play_audio_live_default);
		audioImg.setErrorImageResId(R.drawable.play_audio_live_default);
	...
	private void changeConfig(Configuration newConfig) {
		...
		controllayout = (RelativeLayout) LayoutInflater.from(mActivity).inflate(R.layout.play_audio_controller, null);
	...
	private Handler bindHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case PlayTVConst.TIMESHIFT:
				...
				break;
			case PlayTVConst.AUDIOIMG:
				audioImg.setCoverUrl((String) msg.obj, mActivity);
				break;
			}
		}
	};
