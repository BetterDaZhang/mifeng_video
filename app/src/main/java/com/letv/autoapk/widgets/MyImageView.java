...
import com.letv.autoapk.common.net.GifLoader.GifContainer;
import com.letv.autoapk.common.net.GifLoader.GifListener;
...
public class MyImageView extends ImageView {
	...
	public void setImageUrl(String url, GifLoader gifLoader) {
		mUrl = url;
	...
	void loadGifIfNecessary(final boolean isInLayoutPass) {
		...
		GifContainer newContainer = mGifLoader.get(mUrl,
				new GifListener() {
					...
					@Override
					public void onResponse(final GifContainer response,
							boolean isImmediate) {
						// If this was an immediate response that was delivered
						// inside of a layout
						// pass do not set the image immediately as it will
						...
						if (response.getBitmap() != null) {
							...
							setImageDrawable(response.getBitmap());
						} else if (mDefaultImageId != 0) {
							setImageResource(mDefaultImageId);
						}
					}
