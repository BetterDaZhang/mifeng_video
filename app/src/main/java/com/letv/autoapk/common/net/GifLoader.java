...
public class GifLoader {
     ...
     */
    public static GifListener getImageListener(final ImageView view,
            final int defaultImageResId, final int errorImageResId) {
        return new GifListener() {
            @Override
            ...
            @Override
            public void onResponse(GifContainer response, boolean isImmediate) {
    ...
    public boolean isCached(String requestUrl, int maxWidth, int maxHeight, ScaleType scaleType) {
        ...
        return mCache.getGifDrawable(cacheKey) != null;
    }
     ...
     */
    public GifContainer get(String requestUrl, final GifListener listener) {
     ...
     */
    public GifContainer get(String requestUrl, GifListener imageListener,
    ...
    public GifContainer get(String requestUrl, GifListener imageListener,
        ...
        GifDrawable cachedBitmap = mCache.getGifDrawable(cacheKey);
        if (cachedBitmap != null ){//&& cachedBitmap.getCallback()==null) {
            // Return the cached bitmap.
        	GifContainer container = new GifContainer(cachedBitmap, requestUrl, null, null);
            imageListener.onResponse(container, true);
            return container;
        }
        Logger.e("gifloader", "---------load"+cacheKey);
        // The bitmap did not exist in the cache, fetch it!
        GifContainer imageContainer =
                new GifContainer(null, requestUrl, cacheKey, imageListener);
        ...
        // track it.
        Request<GifDrawable> newRequest = makeImageRequest(requestUrl, maxWidth, maxHeight, scaleType,
                cacheKey);

        mRequestQueue.add(newRequest);
        mInFlightRequests.put(cacheKey,
                new BatchedGifRequest(newRequest, imageContainer));
        return imageContainer;
    }

    protected Request<GifDrawable> makeImageRequest(String requestUrl, int maxWidth, int maxHeight,
            ScaleType scaleType, final String cacheKey) {
        return new GifRequest(requestUrl, new Listener<GifDrawable>() {
            @Override
            public void onResponse(GifDrawable response) {
                onGetImageSuccess(cacheKey, response);
            }
        }, maxWidth, maxHeight, scaleType,  new ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
     ...
     */
    protected void onGetImageSuccess(String cacheKey, GifDrawable response) {
        // cache the image that was fetched.
        mCache.putGifDrawable(cacheKey, response);
    ...
    public class GifContainer {
         ...
         */
        public GifContainer(GifDrawable bitmap, String requestUrl,
    ...
    private class BatchedGifRequest {
        ...
        private GifDrawable mResponseBitmap;
                    ...
                    for (BatchedGifRequest bir : mBatchedResponses.values()) {
                        for (GifContainer container : bir.mContainers) {
                            ...
                            if (bir.getError() == null) {
                                container.drawable = bir.mResponseBitmap;
