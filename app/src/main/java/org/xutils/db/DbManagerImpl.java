...
public final class DbManagerImpl implements DbManager {
	...
	@Override
	public <T> T findBySpecialColum(Class<T> entityType, String specialColumnName, Object idValue) throws DbException {
		TableEntity<T> table = TableEntity.get(this, entityType);
		if (!table.tableIsExist())
			return null;
		Selector selector = Selector.from(table).where(specialColumnName, "=", idValue);
		String sql = selector.limit(1).toString();
		Cursor cursor = execQuery(sql);
		if (cursor != null) {
			try {
	...
	public void addColumn(Class<?> entityType, String column) throws DbException {
		try {
			...
			if (col != null) {
				...
				builder.append("ALTER TABLE ").append("\"").append(table.getTableName()).append("\"").append(" ADD COLUMN ").append("\"")
						.append(col.getColumnName()).append("\"").append(" ").append(col.getColumnDbType()).append(" ").append(col.getProperty());
